# iterates through difference source distances to see how the deposited
# energy of alphas into the SiC detector changes
import argparse  # pass parameters from the command line
import csv  # writes summary to csv file
import re  # modifies text inside config files
import subprocess  # allows to run the allpix script
import sys
...
def set_macro_source(source_mac_path: Path, distance_mm: float) -> None:
    # Scan source distance directly in the Geant4 macro.
    ...
    if count_dir == 0:
        updated += "\n/gps/direction 0 0 -1\n"
    # Keep a narrow cone around the direction axis.
...
def set_conf_output_name(conf_text: str, distance_mm: float) -> str:
    # Keep macro-only scan robust even if config still has a templated source_position.
    updated = re.sub(
    ...
    replacement = (
        f'file_name = "Am241alpha_SiC_collimation_{distance_int}mm.root"'
    )
    if "{DISTANCE}mm.root" in updated:
        return updated.replace(
            'file_name = "Am241alpha_SiC_collimation_{DISTANCE}mm.root"',
            replacement,
        )
    updated, count = re.subn(
        r'^\s*file_name\s*=\s*"Am241alpha_SiC_collimation_.*?mm\.root"\s*$',
        replacement,
        updated,
        flags=re.MULTILINE,
    )
...
# run the allpix script and Extract_Data code
...
def main():
    parser = argparse.ArgumentParser(
        description="Scan source distance and plot mean deposited energy."
    )
    parser.add_argument(
        "--distances-mm",
        nargs="+",
        type=float,
        required=True,
        help="Source distances in mm for /gps/pos/centre z in source.mac.",
    )
    parser.add_argument(
        "--conf",
    ...
    parser.add_argument(
        ...
        help="Geant4 macro containing /gps/pos/centre to scan.",
    )
    parser.add_argument(
        "--run-script",
        default="/home/claire/allpix-squared/SiC_3x3/run_SiC.sh",
        help="Script that runs Allpix and writes deposited_histograms.root.",
    )
    parser.add_argument(
        "--extract-script",
    ...
    parser.add_argument(
        "--outdir",
        default="/home/claire/allpix-squared/SiC_3x3/distance_scan_unconditional",
    ...
    try:
        for d in args.distances_mm:  # loops through the distances
            conf_path.write_text(original_conf)
            source_mac_path.write_text(original_source_mac)

            set_macro_source(source_mac_path, d)
            conf_path.write_text(set_conf_output_name(original_conf, d))
            run_cmd(
                [args.run_script, str(conf_path), str(d)],
                cwd=str(conf_path.parent),
            )

            run_csv = outdir / f"{args.hist_name}_{int(d)}mm.csv"
            run_cmd(
                [
                    sys.executable,
                    args.extract_script,
            ...
            try:
                ...
                print(f"Could not compute unconditional mean at {d} mm: {exc}")
            try:
                ...
                print(f"Could not compute Gaussian sigma at {d} mm: {exc}")
    ...
    finally:  # restores the original files
        conf_path.write_text(original_conf)
        source_mac_path.write_text(original_source_mac)

    summary_csv = outdir / "distance_scan_summary.csv"
    if not rows:
        raise RuntimeError(
            "No distances were processed; nothing to summarize."
        )
    ...
    rows_sorted = sorted(rows, key=lambda r: r["distance_mm"])
    x = np.array([r["distance_mm"] for r in rows_sorted])
    ...
    ax1.set_xlabel("Source distance |z| [mm]")
    ...
    ax1.set_title("3x3 mm2 SiC Distance vs Mean Deposited Energy")
    ax1.legend([h1, h2], ["Mean (all emitted)", "Gaussian sigma (fit)"])
    plt.tight_layout()
    plt.savefig(outdir / "distance_vs_mean_energy.png")
    plt.close()
    ...
    plt.ylim(0, 1.05)
    plt.xlabel("Source distance |z| [mm]")
    plt.ylabel("Depositing fraction")
    plt.title("Distance vs Deposition Efficiency")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(outdir / "distance_vs_efficiency.png")
    plt.close()

    print(f"Wrote summary: {summary_csv}")
    print(f"Wrote plot: {outdir / 'distance_vs_mean_energy.png'}")
    print(f"Wrote plot: {outdir / 'distance_vs_efficiency.png'}")
