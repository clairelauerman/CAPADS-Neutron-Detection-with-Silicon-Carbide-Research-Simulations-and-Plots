# iterates through difference diode bias voltages to see how the deposited
# energy of alphas changes when the detector material is Silicon
import argparse
import csv
import re
import subprocess
...
def set_conf_bias_and_output(
    ...
    replacement = (
        f'file_name = "Am241alpha_Silicon_{distance_int}mm_{bias_int}V.root"'
    )
    ...
    for v in args.bias_voltages_v:
        try:
            ...
            root_out = (
                ...
                / f"Am241alpha_Silicon_{int(round(args.distance_mm))}mm_{int(round(v))}V.root"
            )
...
def main():
    ...
    ax.set_title(
        f"3x3 mm2 SiC Bias Voltage vs Silicon Mean Deposited Energy (distance={args.distance_mm} mm)"
    )
    ...
    fig.tight_layout()
    fig.savefig(outdir / "bias_vs_mean_energy_Silicon.png")
    plt.close(fig)

    print(f"Wrote plot: {outdir / 'bias_vs_mean_energy_Silicon.png'}")
