...
def set_conf_bias_and_output(
    ...
    replacement = f'file_name = "Am241alpha_SiC_metal{tag}_{distance_int}mm_{bias_int}V.root"'
    ...
    print(
        f"SCAN_COMPARISON set_conf function ... Updated ROOT file name is {updated}"
    )
...
def depletion_depth_um(
    bias_v: np.ndarray, sensor_thickness_um: float, full_depletion_v: float
) -> np.ndarray:
    if full_depletion_v <= 0:
        raise ValueError("full_depletion_v must be > 0")
    dep = sensor_thickness_um * (np.clip(bias_v, 0.0, None) / full_depletion_v)
    return np.clip(dep, 0.0, sensor_thickness_um)
...
def run_bias_scan(
    ...
    rows = []
    conf_dump_dir = Path(args.outdir) / "conf_snapshots"
    ...
    for v in args.bias_voltages_v:
        try:
            ...
            # writes the allpix config into a separate file and stores it in conf_snapshot folder
            conf_dump_path = (
                conf_dump_dir / f"SiC_3x3_{tag}_{int(round(v))}V.conf"
            )
            conf_dump_path.write_text(conf_text)
            print(f"[scan:{tag}] snapshot config: {conf_dump_path}")
            ...
            root_out = (
                ...
                / f"Am241alpha_SiC_metal{tag}_{int(round(args.distance_mm))}mm_{int(round(v))}V.root"
            )
...
def main():
    ...
    # path to detector config without the top metal layers
    parser.add_argument(
        "--detector-nometa",
        default="/home/claire/allpix-squared/examples/SiC_3x3_detectorset/SiC_nometal_detector.conf",
        help="Detector file without top metal and Ti layer.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=50.0,
        help="Sensor thickness in um used to map bias voltage to depletion depth.",
    )
    parser.add_argument(
        "--full-depletion-voltage-v",
        type=float,
        default=20.0,
        help="Voltage at which sensor reaches full depletion depth.",
    )
    ...
    detector_nometa = Path(args.detector_nometa)
    ...
    # x_volt = np.array([r["bias_voltage_v"] for r in rows_on])
    # y_ON_sort = sorted(rows_on, key=lambda r: r["mean_all_keV"])
    # y_OFF_sort = sorted(rows_off, key=lambda r: r["mean_all_keV"])
    # y_ON = np.array([r["mean_all_keV"] for r in y_ON_sort])
    # y_OFF = np.array([r["mean_all_keV"] for r in y_OFF_sort])
    # y_volt = np.subtract(y_OFF, y_ON)

    # ax.plot(
    #     x_volt, y_volt, "-o", color="tab:pink", linewidth=2, label="Difference"
    # )

    # # plot mean differences
    # diff_means = np.diff(y)
    # x_means = []
    # for i in range(len(x) - 1):
    #     x_means.append((x[i + 1] + x[i]) / 2)
    ...
    #     color="tab:pink",
    #     linewidth=2,
    #     label="Mean Difference Metal OFF",
    # )

    # # plot depletion depth vs bias voltage
    # x_dep = np.array(sorted(set(args.bias_voltages_v)), dtype=float)
    # y_dep = depletion_depth_um(
    #     x_dep, args.sensor_thickness_um, args.full_depletion_voltage_v
    # )
    # ax2 = ax.twinx()
    # ax2.plot(
    #     x_dep,
    ...
    # ax2.set_ylabel("Depletion depth [um]", color="tab:green")
    # ax2.tick_params(axis="y", labelcolor="tab:green")

    ax.set_title(
        f"3x3 mm2 SiC Bias Voltage vs Mean Deposited Energy (distance={args.distance_mm} mm)"
    )
    ...
    fig.tight_layout()
    fig.savefig(outdir / "bias_vs_mean_energy_metal_comparison.png")
    plt.close(fig)

    print(f"Wrote plot: {outdir / 'bias_vs_mean_energy_metal_comparison.png'}")
